...
def produtos():
    tabelinha = Tk()
    tabelinha.title('TABELA DE PRODUTOS')

    produtinhos = ''

    # Escrevendo os Produtos Existentes
    for x in codigos:

        for nome,valor in x.items():
            coiso = ("({2}) - {0} R${1}".format(nome,valor,(codigos.index(x)+1)))
            produtinhos = produtinhos + '\n' + coiso
            print(coiso)

    # Impressão da Lista
    itens = Label(tabelinha)
    itens['text'] = produtinhos
    itens.grid(column=0, row=0, padx=10, pady=10)

    # Botão para Add Produto
    addProd = Button(tabelinha, text='Adicionar Produto')
    addProd.grid(column=0, row=1, pady=5)

    # Botão para Ret Produto
    retProd = Button(tabelinha, text='Retirar Produto')
    retProd.grid(column=0, row=2, pady=5)
